...
## Default events defined by Godot. We neither need nor want to re-define these in our custom input
## file, so we ignore them.
...
## Reference to the control scheme class script. Used to create new GatoControlScheme instances when
## loading the configuration from the `input.data` file.
